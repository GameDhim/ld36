define(
	...
	, function() {
		console.log("trumpet")
		AudioContext.prototype.createTrumpet = function( options ) {
			console.log( "createTrumpet" )
			...
			if( options != undefined && options.hasOwnProperty( "harmonics" ) ) {
				...
				for( var i = 0 ; i < node.harmonics.length ; i++ ) {
					...
					node.harmonics[i].oscillator.frequency.value = base * (i+1)  * Math.random2( 1, 0.01 ) 

					node.harmonics[i].oscillator.start() 



					node.harmonics[i].adsr = this.createADSR( { attack : 0.1 + ( (i > 0 ) ? 0.1 : 0 ) 
					, decay : 0.1 
					, sustain : 1 
